...
class HUD:
    def __init__(self, player, settings):
        ...
        #Nhạc spear trap
        self.spear_channel = pygame.mixer.Channel(6)
        self.spear_sound = pygame.mixer.Sound(asset("spear_sound.mp3"))
        self.spear_sound.set_volume(0.7)

        #Nhạc khi thắng
        self.win_sound = pygame.mixer.Sound(asset("winning_sound.mp3"))
    ...
    def play_spear_sound(self):
        if self.spear_sound:
            self.spear_channel.play(self.spear_sound)

    def play_win_sound(self):
        if self.win_sound:
            self.game_music_channel.stop()
            self.game_music_playing = False
            self.game_music_channel.play(self.win_sound)
